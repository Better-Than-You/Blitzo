...
import { modlistCache } from '../cache/modlistCache.js'
...
class BotConfig {
  ...
  async createInitialConfig () {
    ...
    await configService.set(
      'bot.creator.jid',
      this.defaults.creator.jid || '',
      'string',
      'Creator WhatsApp JID',
      'setup'
    )
    await configService.set(
      'bot.creator.github',
      this.defaults.creator.github,
      'string',
  ...
  get name () {
    return this.cache.get('bot.name') || this.defaults.name
  }
  get version () {
    return this.cache.get('bot.version') || this.defaults.version
  }
  get description () {
    return this.cache.get('bot.description') || this.defaults.description
  }
  get prefix () {
    return this.cache.get('bot.prefix') || this.defaults.prefix
  }
  get features () {
    return this.cache.get('bot.features') || this.defaults.features
  }
  ...
  async getConfigStats () {
    const totalConfigs = this.cache.size
    const configKeys = Array.from(this.cache.keys())
      .filter(key => key.startsWith('bot.'))
      .map(key => key.replace('bot.', ''))
  ...
  async set (key, value, type = 'string', description = '', updatedBy = 'system') {
    try {
      const success = await configService.set(key, value, type, description, updatedBy)
      if (success) {
        this.cache.set(key, value)
      }
      return success
...
//   }
  ...
  async reset (key, updatedBy = 'system') {
    try {
      const defaultValue = this.defaults[key] || null
      const success = await configService.set(`bot.${key}`, defaultValue, 'string', `Reset by ${updatedBy}`, updatedBy)
      if (success) {
        this.cache.set(`bot.${key}`, defaultValue)
      }
      return success
    } catch (error) {
      logger.error(`Failed to reset config '${key}':`, true, error)
      return false
    }
  }
  ...
  isMod (jid) {
    ...
    return modlistCache.isMod(phoneNumber)
  }

  async addMod (jid, updatedBy = 'system') {
    const phoneNumber = jid.split('@')[0]
    if (!this.isCreator(jid)) {
      return await modlistCache.addMod(phoneNumber, updatedBy)
    }
    return false
  }
  ...
  async removeMod (jid, updatedBy = 'system') {
    const phoneNumber = jid.split('@')[0]
    return await modlistCache.removeMod(phoneNumber, updatedBy)
  }

  async getMods () {
    return modlistCache.getMods()
  }

  async clearMods (updatedBy = 'system') {
    return await modlistCache.clearMods(updatedBy)
  }
}
